...
def cex_balance():
    try:
        ...
        api = cexio.Api(tradeconf.cexiousername, tradeconf.cexiokey, tradeconf.cexiosecret)
        try:
...
def mqsend(message,topic=mq_pubtop):
   ...
   #msg=json.loads('{"timestamp" : "%s", "output" : "%s" }' % (_ts,message_))
    ...
    try:
        publish.single(mq_pubtop_, payload=str(message), hostname=mq_host, port=mq_port,auth = {'username':mq_user, 'password':mq_pass})
    except Exception as err:
...
# so i was thinking, and this could be valid fo either method, something simple like this;
...
while True:
    ...
    try:
        ...
        pass
    if debug: print(cx_bal)
    if debug: print(pb_bal)
    try:
        mqsend(cx_bal, 'cbal')
...
# ok, i dont think it would be very hard to do this in python, but ya, an sh poc could look like this.. 
