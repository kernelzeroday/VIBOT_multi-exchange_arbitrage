...
def cancel_order(exchange,orderID,pair='null'):
    ...
    def cancel_bittrex(order_id):
        ...
        try:
            ...
            if ret == '':
                ...
                if debug: print('[*] Successfully canceled order %s' % order_id)
                return True
            else:
    ...
    def cancel_binance(order_id,pair):
        if debug: print(['*] DEBUG: Binance cancel function pair is:  '+ str(pair))
        origPair = str(pair)
        try:
            pair = pair.split('-')
        except Exception as err:
        ...
        if order_id == 'null':
            eprint('Specify an order_id')
            return False
        try:
            ret = binanceApi.cancel_order(orderId=order_id, symbol=pair)
        except Exception as err:
            logging.info(err)
            eprint('Error canceling binance order: '+ str(err))
            return False
        else:
            ret = json.dumps(ret)
...
def mqParse(client, userdata, message):
   ...
   """
    ...
    if kind == 'Limit' or 'Arbitrage':
        print('[*] Order %s on exchange %s qued in thread for canceling' % (orderID_,exchange_))
