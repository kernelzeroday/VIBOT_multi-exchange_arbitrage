...
def get_ticker(exchange,pair,mode):
    ...
    if exchange == 'poloniex':
        if pair == 'BTC_STR':
            pair == 'BTC_XLM'
    with open('ticker/'+exchange+'/'+pair) as f:
        try:
            ff = f.read()
        except:
            time.sleep(1)
...
def viable(mode,orig,new,c=0):
    ...
    if mode == 'sell':
        ...
        if float(pct) <= 0.1 or float(pct) <= -0.1:
           if debug: print('[i] : Sell Viable:' + str(pct))
           return True
        else:
        ...
        if float(pct) <= 0.1 or float(pct) <= -0.1:
            if debug: print('[i] : Buy Viable: ' + str(pct))
            return True
        else:
            if debug:print("[i] Pct was: " +str(pct))
            return False
...
def que_Order(exchange,expire,orderID,type_,price,pair,qty):
    ...
    rebuy = random.randint(1,5)
    resell = random.randint(1,5)
    rsvt = random.randint(150,300)
    ok=False
    mode = str(type_)
    count=10
    while count > 0:
        if float(expire) <= tS():
            ...
            if mode == 'sell':
                if viable(mode,price,current):
                    mqPublish('[*] Sell Order still viable, not canceling for: %.8f seconds!' % resell)
                    logger.info('Sleeping for %.8f seconds' % resell)
                    time.sleep(resell)
                else:
                    if not ok:
                    ...
                    time.sleep(1)
                    count-=1
                    print('[*] DEBUG: '+str(count) + ' tries left')
                    current = get_ticker(exchange,pair,mode)
                    if float(price) >= float(current):
                        logger.info('[*] DEBUG: Placing sell order...')
                        ret = Exchange(exchange,mode,pair,current,qty)
                        if ret:
                            ret = str(ret)
                            mqPublish('Sell order placed!%s %s %s %s %s'% (exchange,pair,price,qty,ret))
                            time.sleep(30)
                            ret = cancel_order(exchange,ret)
                            if ret:
                                print('[*] Canceled order.')
                            else:
                                mqPublish('[*] Order filled or error canceling.')
                                break
                    else:
                        mqPublish("[*] Sleeping for %.8f, order still not viable." % t)

            elif mode == 'buy':
                if viable(mode,price,current):
                    mqPublish('[*] Buy Order still viable, not canceling for: %.8f seconds!' % vt)
                    logger.info('Sleeping for %.8f seconds' % rebuy)
                    time.sleep(rebuy)
                else:
                    if not ok:
                    ...
                    current = get_ticker(exchange,pair,mode)
                    if float(price) <= float(current):
                        logger.info('[*] DEBUG: Placing Buy order...')
                        ret = Exchange(exchange,mode,pair,current,qty)
                        if ret:
                            ret = str(ret)
                            mqPublish('Buy order placed! %s %s %s %s %s'% (exchange,pair,price,qty,ret))
                            time.sleep(30)
                            ret = cancel_order(exchange,ret)
                            if ret:
                                mqPublish('[*] Canceled order.')
                            else:
                                mqPublish('[*] Order filled or error canceling.')
                                break
                    else:
...
def mqPublish(payload, topic='tracking', qos=0, retain=False):
    ...
    publish.single(topic, payload=payload, hostname=config.mq_host, port=config.mq_port, auth = {'username': config.mq_user, 'password': config.mq_pass})
...
client = mqStart("tracker")
#mq_pubtop='tracking'
# just loop some sh*t to keep this open on the CLI
mqPublish('[*] Starting Order Tracking Engine...')
while 1:
    try:
        time.sleep(0.25)
