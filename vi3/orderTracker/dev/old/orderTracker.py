...
def que_Order(exchange,expire,orderID):
   ...
   """
    ...
    if kind_ == 'Limit':
        ...
        if count>int(maxThreads):
            ...
            t = threading.Thread(target=que_Order, args=(exchange_,expires_,orderID_ ))
            t.start()
            count+=1
        if count % 10 == 0:
...
def mqPublish(payload, topic=mq_pubtop, qos=0, retain=False):
    ...
    """
    global CLIENTS
    client = client('trackerx')
    if not client:
        raise ValueError("Could not find an MQTT Client matching %s" % id)
    if debug:print(payload)
    client.publish(mq_pubtop, payload=payload, qos=qos, retain=retain)
