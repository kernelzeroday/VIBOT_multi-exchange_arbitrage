...
if withdraw:
    if currency == 'XRP' == 'XLM' == 'STR':
        ...
        sys.exit(1)
    if deposit_exchange == 'poloniex' or 'bittrex' or 'cex':
        if exchange == 'bittrex' or 'poloniex':
            print("Withdrawal requested : %s > %s" % (exchange,deposit_exchange))
            print("Getting adderess for %s ..." % deposit_exchange)
            _address = tWrap.deposit_address(deposit_exchange,currency)
            print('Address for %s on %s is: %s' % (currency,deposit_exchange,_address))
            timeStamp=tWrap.tS()
            logging.info("Withdraw initatiated at %s " % timeStamp)
            logging.info('Live: %s Withdraw from: %s Deposit into: %s Currency: %s Quantity: %s Address: %s' % (live,exchange,deposit_exchange,currency,quantity,_address))
            if safe:
                proceed = input('Live: %s Withdraw from: %s Deposit into: %s Currency: %s Quantity: %s Address: %s , procceed (YES/No): ' % (live,exchange,deposit_exchange,currency,quantity,_address))
            else:
                proceed = 'YES'
            if proceed == 'YES' or not safe:
                ret = tWrap._withdraw(live,exchange,currency,quantity,_address)
            ...
            print('Unsupport withdrawal exchange. Valid withdrawl exchanges are %s' % wdex)
            sys.exit(1)
    else:
