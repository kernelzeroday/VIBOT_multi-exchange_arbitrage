...
proceed = input('You requested a withdrawl of %s %s from %s to %s. Proceed? YES/NO : ' %(amount,currency,_from,to))
if proceed == 'YES':
    msg=('\'{"action":"transfer","currency":"%s","amount":%s,"from":"%s","to":"%s"}\'' % (currency,amount,_from,to))
    mqPublish(msg)
