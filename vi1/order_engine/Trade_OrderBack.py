    ...
    def checkBalance(self, side, invPair, price, qty):
        ...
        if minType == "ccxt":
            ...
            if self.name == 'poloniex':
                ...
                ccxtname = '/'.join(ccxtname)
                if ccxtname == 'STR/BTC':
                    ccxtname = ccxtname.replace('STR','XLM')
                pairmarket = marketdata[self.name]
            if self.name == 'cex':
                ccxtname = pairName.split('/')
                ccxtname = '/'.join(ccxtname)
        ...
        return (balance, price, qty, ratio,TOTAL)
    ...
    try:
        ...
        for spread in spreads:
            ...
            if not (buyPair and sellPair):
                ...
                continue
            try:
                buyBal, buyPrice, buyQty, ratio, buyTotal = buyEx.checkBalance("buy", buyPair, spread.BuyPrice, spread.BuyQty)
                sellBal, sellPrice, sellQty, ratio,sellTotal = sellEx.checkBalance("sell", sellPair, spread.SellPrice, spread.SellQty)
                # Cross Check
                if Decimal(sellQty) < Decimal(buyQty):
                    buyBal, buyPrice, buyQty, ratio, buyTotal = buyEx.checkBalance("buy", buyPair, buyPrice, sellQty)
                elif Decimal(buyQty) < Decimal(sellQty):
                    sellBal, sellPrice, sellQty, ratio, sellTotal = sellEx.checkBalance("sell", sellPair, sellPrice, buyQty)
            except ValueError as err:
                ...
                try:
                    ...
                    print(err)
                pubprint('BuyRes '+str(buyRes))
                pubprint('SellRes '+str(sellRes))
                ...
                if LIVE:
                    ...
                    if sellEx.sell(sellPair, sellPrice, sellQty):
                        BUYBACK = False
                        try:
                            buyValue = float(buyRes) * float(buyPrice)
                            cRatio = float(net) * float(buyValue) / float(100.0)
                        except Exception as err:
                            print(err)
                        try:
                            pubprint('BuyEx Ratio : '+str(ratio))
                            pubprint('Current Ratio : ' +str(cRatio))
                        except Exception as err:
                            print(err)
                            pass
                        try:
                            if cRatio <= ratio:
                                ...
                                pubprint('Deny BuyBack: Ratio '+str(cRatio)+' Allocated Ratio: '+str(ratio))
                        except:
                            pass
                    ...
                    # Arb buy
                        if buyEx.buy(buyPair, buyPrice, buyQty):
                            SELLBACK=False
                            try:
                                sellValue = float(sellRes) * float(sellPrice)
                                sRatio = float(net) * float(sellValue) / float(100.0)
                            except Exception as err:
                                print(err)
                            try:
                                pubprint('SellEx Ratio : '+str(ratio))
                                pubprint('Current Ratio : ' +str(sRatio))
                            except:
                                pass

                            try:
                                if sRatio >= ratio:
                                    pubprint('Allow SellBack: Ratio '+str(sRatio)+' Allocated Ratio: '+str(ratio))
                                    SELLBACK = True
                                else:
                                    pubprint('Deny SellBack: Ratio '+str(sRatio)+' Allocated Ratio: '+str(ratio))
                            except:
                                pass
                else:
                ...
                if ORDERBACK and (BUYBACK or SELLBACK):
                    ...
                    try:
                        sellBackBal, sellBackPrice, sellBackQty, ratio, sellTotal = buyEx.checkBalance("sell", buyPair, sellBackPrice, sellBackQty)
                        buyBackBal, buyBackPrice, buyBackQty, ratio, buyTotal  = sellEx.checkBalance("buy", sellPair, buyBackPrice, buyBackQty)
                        # Cross Check
                        if buyBackQty < sellBackQty:
                            sellBackBal, sellBackPrice, sellBackQty, ratio, sellTotal = buyEx.checkBalance("sell", buyPair, sellBackPrice, buyBackQty)
                        elif sellBackQty < buyBackQty:
                            buyBackBal, buyBackPrice, buyBackQty, ratio, buyTotal = sellEx.checkBalance("buy", sellPair, buyBackPrice, sellBackQty)
